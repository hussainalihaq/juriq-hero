...
export const ChatArea: React.FC<ChatAreaProps> = ({ messages, isTyping, onSuggestionClick, onRetry }) => {
    ...
    // State to show/hide the scroll-to-bottom button
    ...
    const handleScroll = () => {
        ...
        // If user is within 150px of bottom, they "stick" to it.
        // If they scroll up more than 150px, stickiness breaks.
        const distanceToBottom = Math.abs(scrollHeight - clientHeight - scrollTop);
        const sticky = distanceToBottom < 150;
    ...
    const scrollToBottom = () => {
        ...
        if (container) {
            ...
            // Re-engage stickiness after manual scroll
            // We set it after a small timeout to allow animation to complete
            setTimeout(() => {
    ...
    return (
        <>
            <div
                ...
                className="flex-1 overflow-y-auto p-4 md:p-12 space-y-8 pb-96 relative"
            >
            ...
            `}>
                ...
                {/* Spacer (Increased to 320px) to ensure scroll past the heavy Input Area */}
                <div className="h-[320px] shrink-0" />
                <div ref={bottomRef} />
            </div>

            {/* Scroll Down Button */}
            <div className={`absolute bottom-32 right-8 z-30 transition-all duration-300 transform ${showScrollButton ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none'}`}>
                <button
                    onClick={scrollToBottom}
                    className="w-10 h-10 rounded-full bg-slate-800 dark:bg-white text-white dark:text-midnight-bg shadow-xl flex items-center justify-center hover:bg-slate-700 dark:hover:bg-slate-200 transition-colors"
